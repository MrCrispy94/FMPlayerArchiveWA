...
import type { Manager, Honour, HonourType, TrophyCabinetSettings } from '../types';
...
interface ManagerTrophyCabinetModalProps {
  isOpen: boolean;
  onClose: () => void;
  manager: Manager;
  allHonours: Honour[];
...
const getTrophyIcon = (honourId: string, allHonours: Honour[], customIcons: Record<string, string>): React.FC<any> | string => {
    ...
    if (honourInfo) {
      if (honourInfo.isManagerOnly) return MedalIcon;
        ...
        <div className="relative pt-4">
            ...
            <div className="flex flex-wrap justify-center items-end gap-x-8 gap-y-12 p-4 min-h-[150px]">
                {honours.map((honour, index) => {
                    ...
                    if (['moty', 'motm'].includes(honour.id) && uniqueLeagues.length === 1 && uniqueLeagues[0]) {
                        ...
                        <div key={`${honour.id}-${index}`} className="flex flex-col items-center group relative" style={{ width: `${trophySize}px` }}>
                             <div className="relative">
                                {typeof IconOrUrl === 'string' ? (
                                    <img src={IconOrUrl} alt={honourName} className="w-full h-auto drop-shadow-lg transition-transform duration-200 group-hover:-translate-y-2" />
                                ) : (
                                    <IconOrUrl className="w-full h-auto drop-shadow-lg transition-transform duration-200 group-hover:-translate-y-2" />
                                )}
                                {honour.count > 1 && (
...
const ManagerTrophyCabinetModal: React.FC<ManagerTrophyCabinetModalProps> = ({ isOpen, onClose, manager, allHonours, customIcons, trophyCabinetSettings }) => {
  ...
  const shelves = useMemo(() => {
    ...
    manager.seasonStats.forEach(stat => {
        for(const honourId in stat.honours) {
            const count = stat.honours[honourId];
            ...
            if (!honoursMap[honourId]) {
                honoursMap[honourId] = {
                    ...
                    leagues: [],
                    count: 0,
                    seasonCounts: info.isCountable ? {} : undefined,
                };
            }
            if (info.isCountable && honoursMap[honourId].seasonCounts) {
                honoursMap[honourId].seasonCounts![stat.season] = (honoursMap[honourId].seasonCounts![stat.season] || 0) + count;
            }
            honoursMap[honourId].seasons.push(stat.season);
            honoursMap[honourId].leagues.push(stat.league);
            honoursMap[honourId].count += count;
        }
    });
  ...
  const chronologicalHonours = useMemo(() => {
    ...
    const allAwards: { season: string; league: string | undefined; honour: Honour; count: number }[] = [];
    manager.seasonStats.forEach(stat => {
        Object.entries(stat.honours).forEach(([honourId, count]: [string, number]) => {
            const honourInfo = getHonourInfo(honourId, allHonours);
            if (honourInfo) {
                allAwards.push({ season: stat.season, league: stat.league, honour: honourInfo, count });
            }
        });
    });
                ...
                <div className="space-y-8 py-4">
                    {Object.keys(shelves).length > 0 ? (
                        ...
                        <div className="flex items-center justify-center h-48 text-gray-500">
                            This manager hasn't won any trophies yet.
                        </div>
                    )}
                </div>
            )}
                        ...
                        <div key={season}>
                            ...
                            <ul className="mt-2 space-y-2 pl-4">
                                {honours.map((item, index) => {
                                    ...
                                    if (['moty', 'motm'].includes(item.honour.id) && item.league) {
                                    ...
                                    return (
                                        <li key={`${item.honour.id}-${index}`} className="flex items-center gap-3 text-gray-300 bg-black/20 p-2 rounded-md">
                                            ...
                                            {item.count > 1 && (
                                                <span className="ml-auto text-xs text-gray-400 font-bold">x{item.count}</span>
                                            )}
                                        </li>
                                    )
                                })}
...
export default ManagerTrophyCabinetModal;
