...
import { League, Club, Competition, SaveGame, HonourType, Honour, TrophyCabinetSettings, NewgenTerm, DateFormatOption } from '../types';
import { CONTINENTS, HONOUR_TYPES, getHonourInfo } from '../constants';
...
const CustomisePage: React.FC<CustomisePageProps> = (props) => {
    const { 
        ...
        newgenTerm, onUpdateNewgenTerm, dateFormat, onUpdateDateFormat,
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    <div className="space-y-6 md:col-span-1">
                        ...
                        <DataListSection title="Add New Club">
                         ...
                         <DataListSection title="Add New League">
                            <form onSubmit={handleAddLeague} className="space-y-3">
                                <div>
                                    <label className="text-sm font-medium text-gray-300 block mb-1">League Name</label>
                                    <Input type="text" value={leagueName} onChange={e => setLeagueName(e.target.value)} placeholder="e.g. Vanarama National League" required/>
                                </div>
                                <div>
                                    <label className="text-sm font-medium text-gray-300 block mb-1">Nation</label>
                                    <Select value={leagueNation} onChange={e => setLeagueNation(e.target.value)}>
                                        {allNations.map(n => <option key={n} value={n}>{n}</option>)}
                                    </Select>
                                </div>
                                <Button type="submit" variant="primary" className="w-full">Add League</Button>
                            </form>
                        </DataListSection>
                    ...
                    <div className="space-y-6 md:col-span-1 lg:col-span-2">
                        ...
                        <DataListSection title="Manage FM Versions">
                            <form onSubmit={handleAddFmVersion} className="flex gap-2 items-end">
                                <div className="flex-grow">
                                    <label className="text-sm font-medium text-gray-300 block mb-1">New Version Name</label>
                                    <Input type="text" value={newFmVersion} onChange={e => setNewFmVersion(e.target.value)} placeholder="e.g. FM26" required/>
                                </div>
                                <Button type="submit" variant="primary">Add</Button>
                            </form>
                            <div className="flex flex-wrap gap-2 pt-2">
                                {fmVersions.map(version => (
                                    <div key={version} className="flex items-center gap-2 pl-3 pr-1 py-1 bg-gray-700/80 rounded-full">
                                        <span className="text-sm text-gray-200">{version}</span>
                                        <button onClick={() => handleDeleteClick('version', version, version, onRemoveFmVersion)} className="text-red-500 hover:text-red-400 transition-colors">
                                            <TrashIcon className="w-4 h-4" />
                                        </button>
                                    </div>
                                ))}
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            <DataListSection title="Custom Awards & Trophies">
                             ...
                             <DataListSection title="Custom Clubs">
                                <div className="space-y-2 max-h-60 overflow-y-auto pr-2">
                                    {customClubs.map(club => (
                                        <div key={club.id} className="flex justify-between items-center p-2 bg-gray-700/60 rounded-md">
                                            <span>{club.name}</span>
                                            <button onClick={() => handleDeleteClick('club', club.name, club.id, onRemoveClub)} className="text-red-500 hover:text-red-400 transition-colors"><TrashIcon className="w-4 h-4" /></button>
                                        </div>
                                    ))}
                                </div>
                            </DataListSection>
